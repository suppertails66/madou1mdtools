  ...
  int MdPattern::fromGrayscaleGraphic(const BlackT::TGraphic& src,
    ...
    for (int j = 0; j < h; j++) {
      for (int i = 0; i < w; i++) {
        ...
        int raw = color.r() / 16;
        if (color.a() == TColor::fullAlphaTransparency) raw = 0;
  ...
  int MdPattern::fromColorGraphic(const BlackT::TGraphic& src,
    ...
    for (int j = 0; j < h; j++) {
      for (int i = 0; i < w; i++) {
        ...
        TColor color = src.getPixel(x, y);
//        std::cerr << dec << x << " " << dec << y << " " << hex << (int)color.r() << " " << hex << (int) color.g() << " " << hex << (int) color.b() << " " << hex << (int) color.a() << endl;
...
//    dst.resize(w, h);
  ...
  void MdPattern::read(BlackT::TStream& ifs) {
    for (int j = 0; j < MdPattern::h; j++) {
      for (int i = 0; i < MdPattern::w; i += 2) {
        unsigned char b = (unsigned char)(ifs.get());
