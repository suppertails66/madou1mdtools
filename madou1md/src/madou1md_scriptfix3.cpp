...
//    if (line.size() <= 0) continue;
    ...
    while (!ifs.eof()) {
      ...
      if (ifs.peek() == '[') {
        while (ifs.peek() != ']') ofs.put(ifs.get());
        ofs.put(ifs.get());
      }
      else if (ifs.peek() == '{') {
          ofs.put(ifs.get());
        ...
        if (doubleQuoteActive) ofs.put('<');
        else {
          ofs.put('{');
          string str;
          str.assign(ifs.data().data(), ifs.data().size());
          cout << "correcting at line " << lineNum << ": "
            << str << endl;
        }
      }
      else if (ifs.peek() == '>') {
...
int main(int argc, char* argv[]) {
  if (argc < 4) {
    cout << "Madou Monogatari I (Mega Drive) script fixer" << endl;
